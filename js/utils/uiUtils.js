...
export function updateSourcePreviewTransform(elements, state) {
    ...
    // Calculate bounds - allow image center to reach container edge
    ...
    // Use different variables for clarity during clamping
    let clampedX = currentOffsetX;
    let clampedY = currentOffsetY;

    if (scaledWidth <= containerWidth) {
        // If image is narrower than container, center it horizontally
    ...
    if (scaledHeight <= containerHeight) {
        // If image is shorter than container, center it vertically
...
export function resetState(
    ...
    const {
        ...
        // Pre-effect controls (only those needing direct value reset)
         preEffectIntensitySlider, preEffectWaveAmplitudeSlider,
    ...
    state.lastAppliedImageData = null;
...
export function panMove(event, elements, state, updateSourcePreviewTransformFunc, handleSliderChangeFunc) {
    ...
    if (typeof updateSourcePreviewTransformFunc === 'function') {
        ...
        if (state.currentOffsetX !== clampedX || state.currentOffsetY !== clampedY) {
         ...
         console.warn('[uiUtils.panMove] updateSourcePreviewTransformFunc not provided or not a function.');
    }

    // Optionally call the general slider change handler if panning should trigger preview updates
    if (typeof handleSliderChangeFunc === 'function') {
         // console.log('[uiUtils.panMove] Calling handleSliderChangeFunc...'); // Verbose
         handleSliderChangeFunc();
    }
}
...
export function endPan(elements, state) {
    ...
    if (!state?.isDragging) {
         console.log('[uiUtils.endPan] Was not dragging, doing nothing.');
         return; // Ignore if not dragging
    }
    state.isDragging = false;
    ...
    if (elements?.sourcePreviewContainer) {
        elements.sourcePreviewContainer.style.cursor = 'grab'; // Use 'grab' for movable state
        console.log('[uiUtils.endPan] Set cursor to grab.');
    }
}
...
export function handleSourceZoom(elements, state, updateSourcePreviewTransformFunc, handleSliderChangeFunc) {
    ...
    // Trigger general update if zooming affects the final output preview
    if (typeof handleSliderChangeFunc === 'function') {
...
/**
 * Sets up an input event listener for a slider to update a display element
 * and optionally call a callback function.
 ...
 * @param {Function} callback - Function to call on slider input.
 * @param {Function} [formatter=val => val] - Function to format the slider value for display.
 */
